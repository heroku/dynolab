...
func TestNATForwarding(t *testing.T) {
	t.Parallel()
...
func TestNATShutdown(t *testing.T) {
	t.Parallel()
	...
	lnNAT, err := bridge.Listen("tcp", "127.0.0.1/32:0")
	if err != nil {
		t.Fatal(err)
	}
	...
	case err := <-runc:
		...
		t.Fatal("NAT.Run finished while connections are still open")
	}
